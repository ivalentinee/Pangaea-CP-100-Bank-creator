...
function create_preset {
    local file=$1
    local bank_folder_name="$(printf "%s_%s" $BANK_FOLDER_NAME $2)"
    local preset_folder_name=Preset_$3

    mkdir -p "$RESULTS_FOLDER/$bank_folder_name/$preset_folder_name"
    convert_file "$file" "$RESULTS_FOLDER/$bank_folder_name/$preset_folder_name/$file"
}
